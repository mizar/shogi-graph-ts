...
const castle = (require("./castle.json") as {
    id: string; // タグ管理名
    name: {
        ja_JP: string; // 日本語名
        en_US?: string; // 英語名
    ...
    >((r, e) => {
        if (e.noturn) {
            ...
            const revSquare = (sq: string) =>
                sq
                    .replace(/[1-9]/, (c) =>
                        String.fromCharCode(106 - c.charCodeAt(0))
                    )
                    .replace(/[a-i]/, (c) =>
                        String.fromCharCode(202 - c.charCodeAt(0))
                    );
            r.push({
                id: `SENTE_${e.id}`,
                name: {
            ...
            r.push({
                ...
                moves: e.moves?.map((m) => {
                    ...
                    return _m
                        ? `${revPiece(_m[1])}*${revSquare(_m[2])}${revSquare(
                              _m[3]
                          )}${_m[4]}`
                        : "";
                }),
    ...
    .map((e) =>
        Object.assign({}, e, {
            pieces_readable: e.pieces?.map((p) => {
                ...
                return _m
                    ? `${turn_readable(_m[1])}${sq_readable(
                          _m[2]
                      )}${piece_readable(_m[1])}`
                    : "";
            }),
            hand_readable: e.hand?.map((p) => {
                const _m = p.match(/^(\+?[PLNSGBRKplnsgbrk])$/);
                return _m
                    ? `${turn_readable(_m[1])}${piece_readable(_m[1])}`
                    : "";
            }),
            ...
            moves_readable: e.moves?.map((m) => {
                ...
                return _m
                    ? `${turn_readable(_m[1])}${sq_readable(
                          _m[2]
                      )}${piece_readable(_m[1])}→${turn_readable(
                          _m[1]
                      )}${sq_readable(_m[3])}${piece_readable(
                          `${_m[4]}${_m[1]}`
                      )}`
                    : "";
            }),
        ...
        castle.forEach((c) => {
            if (
                !(
                    ...
                    p.some((t) => t.id === c.id) ||
                    (c.tags_required &&
                        c.tags_required.some(
                            (te) => !p.some((_te) => _te.id === te)
                        )) ||
                    c.pieces?.some((p) => {
                        const pm = p.match(
                            /^(\+?[_PLNSGBRKplnsgbrk])\*([1-9])([a-i])$/
                ...
                const tag = Object.assign({}, c, {
                    ...
                    hide:
                        c.hide ||
                        c.tags_exclude?.some((te) =>
                            p.some((_te) => _te.id === te)
                        ) ||
                        undefined,
                });
